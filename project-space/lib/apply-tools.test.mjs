...
test("set_editable: CCT 改暖", () => {
  const r = applyTools(baseState(), [
...
test("switch_region: HK → JP · 更严合规触发 fail", () => {
  const r = applyTools(baseState(), [
...
test("switch_variant: 有 variantLoader 会 overlay", () => {
  const variantData = {
...
test("switch_variant: 未知 variant 拒绝", () => {
  const r = applyTools(baseState(), [
...
test("append_timeline 覆盖 auto-timeline", () => {
  const r = applyTools(baseState(), [
    { name: "set_editable", args: { field: "lighting_cct", value: 2700 } },
...
test("无改动：state 不 recompute · timeline 不加", () => {
  const r = applyTools(baseState(), [
