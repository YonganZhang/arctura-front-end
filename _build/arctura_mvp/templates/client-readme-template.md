# CLIENT-README.md 模板（含 5 stakeholder 视角 + 设计统计）

> 每个 MVP 输出文件夹必含一份 `CLIENT-README.md`，使用此模板。
> 占位符用 `{...}` 标记，agent 填入具体值。
...
```markdown
# {项目中文名} — 方案文件指南
> 客户/合伙人专用 · {YYYY-MM-DD} · Studio Copilot v3
...
| 3D 网页预览 | exports/{slug}.glb | ... | 客户 / 营销 |
| 3D 通用 | exports/{slug}.obj | ... | 3D 设计师 |
| 3D 动画 | exports/{slug}.fbx | ... | 动画 / Unity 团队 |
| BIM | exports/{slug}.ifc | ... | BIM 工程师 |
...
### 5️⃣ BIM 工程师 / 结构机电
**主要看**: `exports/{slug}.ifc` (IFC4 BIM 模型)
**软件**: Revit / ArchiCAD / BlenderBIM (Blender 插件，免费) / Solibri
**能干**: 结构/机电协同、碰撞检测、工程量算量、设备联动
**3D 备份**: `exports/{slug}.glb / .obj / .fbx` 可导入 SketchUp / Rhino / Max / Maya / Unity
...
- **{slug}.glb**: 给客户 web 预览 (gltf-viewer.donmccurdy.com 拖入)
- **{slug}.obj**: 给 3D 设计师 (SketchUp/Rhino)
- **{slug}.fbx**: 给动画师 (Maya/Unity)
- **{slug}.ifc**: 给 BIM 工程师 (Revit/ArchiCAD)
