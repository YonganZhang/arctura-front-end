...
| 9.3 EnergyPlus binary + 真能耗 simulate + compliance-HK.md | ✅ | 装 EnergyPlus 25.1 Ubuntu22.04 至 `~/.local/EnergyPlus-25.1.0/` · `~/.arctura-env` 加 PATH + `ENERGYPLUS_IDF_VERSION=25.1` · 修严老师 `idf_builder.py` 两处：Version 硬编码 → env 驱动、`For: Weekends Holidays` → `For: AllOtherDays`（EP 25.1 语法）· `energy_report.py` 加 Step 1.5 `run simulate --weather HK.epw` · 真 EUI 46 kWh/m²·yr · 真 compliance-HK.md 8 check（OTTV/LPD/SHGC…）· full 档 E2E 11/11 真产 |
