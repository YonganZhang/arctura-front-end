...
| 11.1 scene generator hybrid-fallback 修 + 双 SSOT 架构 ADR | ✅ 2026-05-04 | 用户报告"任何 brief 都生成同一个 22㎡ 10 obj 3 灯 scene"。深扫发现 scene generator 严格 dict lookup 把 LLM 自创 type（`hybrid cafe-office`/`校长办公室`/`principal office`）全 fallback 到 default · 永远 3 件家具。同时暴露 brief schema 5 处副本 / scene-ops 100% 单向 / fe_payload 死字段等架构债。Codex v1 6/10 → v2 7/10 → Step1 8/10 三审通过。Step 1 ship：双语 resolver（Python+JS 关键词词边界匹配 + 共享 fixture · 10 enum × 多别名）· `_DEFAULTS_BY_TYPE` 补 retail/clinic/gallery/multipurpose · API 边界 `normalize-brief.js` 写 `space.{type, type_raw, resolved_types}` 三段 · `_shared/project-name.js` 同步 display_name 兼容 4 种历史形状 · brief-rules.json 加 enum + project_shape · 测试 +90 case (Python 157/JS 42 全绿) · 实战 `hybrid cafe-office` 从 10 obj → 34 obj 7 主家具（cafe ∪ office）。架构契约：[ADR-001 cache-vs-ssot](../../wiki-methodology/top/decisions/ADR-001-cache-vs-ssot-model.md) · [ADR-002 brief.project shape](../../wiki-methodology/top/decisions/ADR-002-brief-project-name-shape.md) |
| 11.2-11.5 双 SSOT 落地（registry kind + baseline + derive + overrides）| ✅ 2026-05-04 | 接 Phase 11.1 ADR 落地 · 4 子 step：(11.5) `product_registry` 加 `kind` 字段 + `resolve_tier_products_by_kind` helper · 4 类（input/derive_input/fast_artifact/slow_artifact）· (11.2) `_build/scripts/derive_baseline.py` read-only 52 MVP canonical-subset snapshot + 浮点容忍 diff（epsilon 1e-3）· 跑出 only 3/52 MVP 有真 scene · (11.3) `_build/arctura_mvp/derive/__init__.py` 纯函数 `derive(brief, overrides, artifacts_index)` → `DeriveBundle{scene, editable, derived_metrics, _signatures}` · 启发式 metrics（保温越好 EUI 越低）· artifacts_index 真 EUI 覆盖启发式 · (11.4) `api/_shared/{scene-ops-mapping, overrides-apply}.js` JS 双语对称实现 + Python `derive/overrides.py` · 4 namespace（layout/structural/appearance/lighting + tombstones）覆盖 16 op · 新 endpoint `/api/projects/[slug]/overrides` GET/POST/DELETE+ns reset · 新 KV key `K.projectOverrides(slug)`。**测试 +61 case**（总 Python 195/JS 61 全绿 = 256）。 |
| 11.6 palette 同类塌缩修 + 9 differential 元测试 | ✅ 2026-05-04 | subagent 扫到 9 处潜在塌缩 · 致命 #1：style.keywords 4 preset → 7 preset · 加 `_PALETTE_KEYWORDS` 关键词包含匹配 · `tests/test_no_collapse_invariants.py` 锁住"不同输入必产不同输出" |
| 11.7 一劳永逸 ADR-003 · Resolver 注册表 + lint 守卫 | ✅ 2026-05-04 | 不再单点修补 · 引入结构性方案：`Resolver` 抽象 + 中央注册表 + 元测试 harness（自动遍历 6 类不变式）+ `_build/scripts/lint_fallbacks.py` 静态扫所有 `dict.get(x, "literal")` · 4 个 resolver 注册（space_type / region / building_category + 2 mapping）· 修剩 2 致命（cost_per_m2 region · light_by_cat category）· [ADR-003](../../wiki-methodology/top/decisions/ADR-003-resolver-registry-pattern.md) |
| 11.8 修 Claude Sonnet ```json fence 报错 | ✅ 2026-05-04 | 用户切 Sonnet 立即炸 `Unexpected token '\``。Claude 包 fence · GPT-5 走 strict json mode。建跨语言鲁棒解析器 `api/_shared/llm-parse-json.js` + `_build/arctura_mvp/chat/llm_parse_json.py` · 4 策略级联（strict / fence / 平衡块 / cleanup）· 跨语言 fixture 锁 10 真实模型场景 |
| 11.9 修 max_tokens 截断 + parser 防内部子对象误识别 | ✅ 2026-05-04 | 用户跑到第 4 轮 brief chat 末尾被腰斩。max_tokens 2000 → 4096 · text 以 `{` 开头时跳过策略 3 防 parser 误返子对象 · finish_reason 显式诊断 · fixture 加 truncation case 锁回归 |
| 11.10 修 'LLM: internal error' 模糊错误 · gateway 8 层防御 | ✅ 2026-05-04 | 加 8 层独立诊断：LLM_KEY 缺 / 25s timeout / 网络失败 / HTTP 非 ok / 非 JSON body / choices 缺 / content 空 / 外层 catch error_name + stack_tail · 10 个 gateway 错误路径单测 · **总测试 358 全绿** |
| **Phase 12 待做**（**新会话执行**）兼容郭老师上游 | ⏳ | 郭直灵老师（GitHub `Chokurei` · 历史 wiki 误称"严老师"已纠正）在 `Arctura-Lab/Pipelines` 4-21→4-28 做了 6 天 + Internal Handbook v6.0 · 你本地 `StartUP-Building/` 4-20 起脱节。详 [findings/teacher-repo-sync-status-2026-05-04](../../wiki-methodology/top/findings/teacher-repo-sync-status-2026-05-04.md) |
