...
async function kv(cmd, ...args) {
  ...
  try {
    ...
    const err = new Error(`Upstash network: ${e.message || e}`);
    err.code = "KV_NETWORK"; err.op = cmd; err.key = args[0]; throw err;
  }
  if (!r.ok) {
    const err = new Error(`Upstash HTTP ${r.status}`);
    err.code = "KV_HTTP"; err.status = r.status; err.op = cmd; err.key = args[0]; throw err;
  }
...
export default async function handler(req) {
  ...
  (async () => {
    ...
    try {
      // 1. job meta 快照
      let metaRaw;
      try { metaRaw = await kv("get", K.job(jobId)); }
      catch (e) {
        await safeWrite(sseMessage("error", {
          where: "api.jobs.stream.kv_get_meta",
          code: e.code || "KV_ERROR",
          message: e.message,
          context: { job_id: jobId, op: e.op, key: e.key, status: e.status },
          retryable: true,
          hint: "Upstash KV 网络抖动 · 刷新页面重试",
        }));
        await safeWrite(sseMessage("stream_end", { reason: "kv_error" }));
        return;
      }
      if (!metaRaw) {
        await safeWrite(sseMessage("error", {
          where: "api.jobs.stream.lookup",
          code: "JOB_NOT_FOUND",
      ...
      let consecutiveKvErr = 0;
      const KV_ERR_WARN_AFTER = 5;  // 连续 5 次 lrange 失败 → 告诉前端
      ...
      while (!closed && !terminalSeen) {
        ...
        try {
          ...
          // KV 偶发错误 · 不致命 · 下一轮重试 · 但连续多次失败要告诉前端
          consecutiveKvErr += 1;
          if (consecutiveKvErr === KV_ERR_WARN_AFTER) {
            await safeWrite(sseMessage("kv_unstable", {
              where: "api.jobs.stream.lrange",
              code: e.code || "KV_ERROR",
              message: `连续 ${consecutiveKvErr} 次 KV 读失败 · ${e.message}`,
              context: { job_id: jobId, cursor, op: e.op },
              hint: "进度暂停推送 · KV 恢复后会自动追上 · 不影响 worker 后台跑",
            }));
          }
          await sleep(POLL_MS);
          continue;
      ...
      await safeWrite(sseMessage("error", {
        where: "api.jobs.stream.exception",
